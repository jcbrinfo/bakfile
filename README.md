...
* `contexts/`: Docker build contexts generated by the `Makefile`.

* `src/tk.bakfile.users/root/rsync-users`: List of the SSH users. All lines must
...
* `src/tk.bakfile.users/root/ssh-auth-keys/`: The initial SSH authentication
...
* `src/tk.bakfile.rsync/sshd_config`: Used by `tk.bakfile.rsync` as the
...
* `src/`: The Docker build contexts, without any customization.
